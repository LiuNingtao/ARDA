...
from dataset_skull_mask import DataSetAgeFusion as MyDataSet
...
parser.add_argument('--data_dir',
                    type=str,
                    default='/media/gsp/LNT/DataSet/Tooth/SKULL_labeled_mini/')
parser.add_argument('--result_path', type=str, default=os.path.join(curr_dir, './result/result0119_skull_mini'))
parser.add_argument('--load_path', type=str, default=os.path.join(curr_dir, './result/result0119_skull_mini'))
...
parser.add_argument('--desc', type=str, default='vertebra+ label mini')
...
def entrance():
    ...
    data_set_dir = os.path.join(args.data_dir, 'data_set')

    train_df = pd.read_csv(os.path.join(data_set_dir, 'train.csv'))
    test_df = pd.read_csv(os.path.join(data_set_dir, 'test.csv'))
    ...
    transform_skull = transforms.Compose([
        Enhancement(),
        FixShape(),
        ToImage(),
    ...
    train_dataset = MyDataSet(train_df, args.data_dir, transform_skull, transform_mask, mode='train')
    test_dataset = MyDataSet(test_df, args.data_dir, transform_skull, transform_mask, mode='test')
