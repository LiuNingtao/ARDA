...
class DataSetAgeFusion(Dataset):
    ...
    分别进行预处理并且进行数据增广
    """
    def __init__(self, data_df, data_dir, heat_dir, transformer_skull, transformer_mask, mode='train', fusion_mode='channel', part='vertebra'):
        ...
        self.part = part
        self.data_df = self._filter_dataset(data_df)
    
    def _filter_dataset(self, data_df):
        
        npy_dir = os.path.join(self.heat_dir, self.mode)
        npy_list = []
        for root, _, file_list in os.walk(npy_dir):
            npy_list.extend(file_list)
        
        def is_name_in(x):
            file_name = x['filename'].split('/')[8].replace('.nii', '.npy')
            if file_name in npy_list:
                return True
            return False
        filtered_df = data_df[data_df.apply(is_name_in, axis=1)]
        return filtered_df
    ...
    def __getitem__(self, index: int):
        try:
            ...
            age = float(self.data_df.iloc[index]['label'])
            (skull_age, skull_name) = skull_path.split('/')[-2:]
            skull_path = os.path.join(self.data_dir, str(skull_age), skull_name.replace('.nii', '_{}.nii'.format(self.part)))
            heat_path = os.path.join(self.heat_dir, self.mode, str(skull_age), skull_name.replace('.nii', '.npy'))
        ...
        if self.transformer_mask:
            heat_data = self.transformer_mask(heat_data)

        assert heat_data.shape[0] ==  1, print('Shape Error')
        assert skull_data.shape == heat_data.shape, print('Shape Not Equal Error')
        ...
        if self.fusion_mode == 'channel':
            ...
            skull_data = torch.cat([skull_data, heat_data], dim=0)
