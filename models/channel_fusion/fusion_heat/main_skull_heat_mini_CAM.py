...
from dataset_skull_heat_trans import DataSetMiniCAM as MyDataSet
...
parser.add_argument('--result_path', type=str, default=os.path.join(curr_dir, './result/result0313_mini_CAM'))
parser.add_argument('--load_path', type=str, default=os.path.join(curr_dir, './result/result0313_mini_CAM'))
parser.add_argument('--pertrained_path', type=str, default=r'')
...
parser.add_argument('--desc', type=str, default='使用一部分数据训练模型生成CAM，体现弱监督')
    ...
    for epoch in range(init_epoch, args.epoch_num):
        ...
        if (epoch) % args.val_freq == 0:
            ...
            if loss_avg_val <= min_loss_val:
                ...
                not_improve_time += 1
                if not_improve_time > 3 and epoch>20:
                    break
...
def entrance():
    ...
    train_dataset = MyDataSet(train_df, args.pertrained_path, train_transform, train_transform, mode='train')
    test_dataset = MyDataSet(test_df, args.pertrained_path, test_transform, test_transform, mode='test')
    val_dataset = MyDataSet(val_df, args.pertrained_path, test_transform, test_transform, mode='val')
