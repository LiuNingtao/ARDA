...
from dataset_single_skull import DataSetAgeFusion as MyDataSet
...
parser.add_argument('--data_path',
                    type=str,
                    default='/media/gsp/48cfceb8-8b77-4141-bba7-da05abd58d95/2019/lnt/project/ToothAge/DataSet/AgeDataSetSingleSkull/DataSetBalanced')
parser.add_argument('--result_path', type=str, default=os.path.join(curr_dir, './result/result_bal_verify'))
parser.add_argument('--load_path', type=str, default=os.path.join(curr_dir, './result/result_bal_verify'))
...
parser.add_argument('--desc', type=str, default='数据平衡实验')
...
def train_model(model,
    ...
    for epoch in range(init_epoch, args.epoch_num):
        ...
        np.save(os.path.join(result_save_path, 'label_{}_train'.format('last')), np.array(label_list))
        np.save(os.path.join(result_save_path, 'output_{}_train'.format('last')), np.array(output_list))
        ...
        if loss_avg < min_loss_train:
            ...
            np.save(os.path.join(result_save_path, 'label_{}_train'.format('best')), np.array(label_list))
            np.save(os.path.join(result_save_path, 'output_{}_train'.format('best')), np.array(output_list))
        ...
        if (epoch) % args.val_freq == 0:
            ...
            if loss_avg_val < min_loss_val:
                ...
                np.save(os.path.join(result_save_path, 'label_{}_val'.format('best')), np.array(label_list_val))
                np.save(os.path.join(result_save_path, 'output_{}_val'.format('best')), np.array(output_list_val))
        ...
        if is_test:
            ...
            if loss_avg_test < min_loss_test:
                ...
                np.save(os.path.join(result_save_path, 'label_{}_test'.format('best')), np.array(label_list_test))
                np.save(os.path.join(result_save_path, 'output_{}_test'.format('best')), np.array(output_list_test))
...
def entrance():
    ...
    #     model = model.cuda()
    if args.is_pertrain:
        model = EfficientNet.from_pretrained('efficientnet-b0', in_channels=args.in_channels)
    else:
    ...
    train_df = pd.read_csv(os.path.join(args.data_path, 'train_balance.csv'))
    val_df = pd.read_csv(os.path.join(args.data_path, 'val_balance.csv'))
    test_df = pd.read_csv(os.path.join(args.data_path, 'test_balance.csv'))

    train_dataset = MyDataSet(train_df, train_transform_skull, mode='train')
    test_dataset = MyDataSet(test_df, test_transform_skull, mode='test')
    val_dataset = MyDataSet(val_df, test_transform_skull, mode='val')
