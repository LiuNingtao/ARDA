...
from inception import inception_resnet_v1, inception_resnet_v2, inception_v4
from efficientnet_pytorch import EfficientNet
from dataset_inception import DataSetAgeFusion as MyDataSet
...
parser.add_argument('--data_path',
                    type=str,
                    default='/media/gsp/48cfceb8-8b77-4141-bba7-da05abd58d95/2019/lnt/project/ToothAge/DataSet/AgeDataSetSingleSkull/DataSetFilteredMini')
parser.add_argument('--result_path', type=str, default=os.path.join(cur_dir, './result/1202_v4'))
parser.add_argument('--load_path', type=str, default=os.path.join(cur_dir, './result/1202_v4'))
...
parser.add_argument('--desc', type=str, default='单个skull,renet18')
...
def train_model(model,
    ...
    for epoch in range(init_epoch, args.epoch_num):
        ...
        if (epoch) % args.val_freq == 0:
            ...
            np.save(os.path.join(result_save_path, 'label_{}_val'.format(str(epoch))), np.array(label_list_val))
            np.save(os.path.join(result_save_path, 'output_{}_val'.format(str(epoch))), np.array(output_list_val))
...
def entrance():
    ...
    model = inception_v4(classes=args.num_class)
