...
from dataset_skull_heat_trans import DataSetAgeFusion as MyDataSet
from model_fusion import HeatFusion
...
parser.add_argument('--heat_dir',
                    type=str,
                    default='/media/gsp/LNT/DataSet/Tooth/heat-map-40-1/heat/')
parser.add_argument('--result_path', type=str, default='/media/gsp/48cfceb8-8b77-4141-bba7-da05abd58d95/2019/lnt/project/ToothAge/models/block_fusion/result/1023')
parser.add_argument('--load_path', type=str, default='/media/gsp/48cfceb8-8b77-4141-bba7-da05abd58d95/2019/lnt/project/ToothAge/models/block_fusion/result/1023')
parser.add_argument('--load_old', type=bool, default=False, choices=[True, False])

parser.add_argument('--print_freq', type=int, default=1)
parser.add_argument('--val_freq', type=int, default=1)
parser.add_argument('--test_freq', type=int, default=10)
parser.add_argument('--init_lr', type=float, default=0.00002)
parser.add_argument('--drop_out', type=float, default=0.5)
parser.add_argument('--exchange_factor', type=float, default=0.2)
parser.add_argument('--result_factor', type=float, default=0.2)
...
parser.add_argument('--model', type=str, default='efficient-block')
parser.add_argument('--fusion_mode', type=str, default='blcok', choices=['channel', 'block', 'head'])
parser.add_argument('--desc', type=str, default='skull + heat, 分别进行transform并且进行数据增广 block')
...
def train_model(model,
    ...
    for epoch in range(init_epoch, args.epoch_num):
        ...
        for data in tqdm(train_data_loader):
            skull_data, heat_data, label = data['skull'], data['heat'], data['label']
            # inputs = torch.cat([inputs for i in range(3)], dim=1)
            # print(inputs.size())
            skull_data, heat_data, label = Variable(skull_data), Variable(heat_data), Variable(label)
            if USE_CUDA:
                skull_data, heat_data, label = skull_data.cuda(), heat_data.cuda(), label.cuda()
            optimizer.zero_grad()
            heat_output, skull_output = model({'skull': skull_data, 'heat': heat_data})
            outputs = (1-args.result_factor) * skull_output + args.result_factor * heat_output
        ...
        if (epoch) % args.val_freq == 0:
            ...
            if loss_avg_val < min_loss_val:
                ...
                np.save(os.path.join(result_save_path, 'output_best_val'.format(str(epoch))), np.array(output_list_val))
...
def entrance():
    ...
    # if USE_CUDA:
    #     model = model.cuda()
    model = HeatFusion(exchange_factor=args.exchange_factor)
    if USE_CUDA:
        model = model.cuda()
    # print(model)
    ...
    train_dataset = MyDataSet(train_df, args.heat_dir, train_transform, train_transform, mode='train')
    test_dataset = MyDataSet(test_df, args.heat_dir, test_transform, test_transform, mode='test')
    val_dataset = MyDataSet(val_df, args.heat_dir, test_transform, test_transform, mode='val')
